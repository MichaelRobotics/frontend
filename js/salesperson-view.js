...
const SalespersonView = (() => {
    ...
    let detailsMeetingTitle, detailsMeetingDate, detailsClientEmail, detailsMeetingStatus, detailsClientCode, detailsRecorderLinkAnchor, detailsMeetingNotes, editMeetingBtn, deleteMeetingBtn;
    ...
    function getHTML() {
        ...
        <header class="bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-xl sticky top-0 z-40">
            <div class="container mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row justify-between items-center">
                ...
                <div id="user-controls-sales" class="fade-in" style="--delay: 1;">
        ...
        <main class="flex-grow container mx-auto p-5 sm:p-8">
            <div id="meeting-list-view-sales" class="view-section max-w-4xl mx-auto fade-in" style="--delay: 0.2;">
    ...
    function initDOMReferences() {
        // Query elements within the #salesperson-view-container
        const viewContainer = document.getElementById('salesperson-view-container');
        if (!viewContainer) {
            console.error("Salesperson view container not found in DOM during initDOMReferences.");
            return;
        }
        ...
        meetingIdInput = viewContainer.querySelector('#meeting-id-sales-hidden'); // Corrected ID
        meetingTitleInput = viewContainer.querySelector('#meeting-title-sales-input'); // Corrected ID
        meetingDateInput = viewContainer.querySelector('#meeting-date-sales-input'); // Corrected ID
        clientEmailInput = viewContainer.querySelector('#client-email-sales-input'); // Corrected ID
        meetingNotesInput = viewContainer.querySelector('#meeting-notes-sales-input'); // Corrected ID
        ...
        detailsRecorderLinkAnchor = viewContainer.querySelector('#details-recorder-link-sales'); // This is an <a> tag now
    ...
    function showSalesView(viewName) {
        if (!meetingListView || !newEditMeetingView || !meetingDetailsView || !backToListBtn) {
            // console.error("Salesperson sub-view elements not found. DOM not ready or incorrect IDs.");
            return;
        }
    ...
    function renderSalesMeetingList() {
        ...
        // Filter meetings: Salesperson should only see meetings they created or are relevant to them.
        // For this demo, we assume all meetings in the shared array are for this salesperson.
        // In a real app, meetings would have a salesperson_id or similar.
        const salespersonMeetings = meetings; // Or filter if meetings have owner ID

        if (salespersonMeetings.length === 0) {
            noMeetingsMessage.classList.remove('hidden');
            return;
        }
        noMeetingsMessage.classList.add('hidden');
        salespersonMeetings.sort((a, b) => new Date(b.date) - new Date(a.date)); // Sort by date descending
        ...
        salespersonMeetings.forEach((meeting, index) => {
            ...
            item.style.setProperty('--delay', `${index * 0.05}s`); // Ensure 's' unit
            ...
            // Add other statuses if necessary (e.g., 'Recording', 'Processing' if salesperson needs to see them)
    ...
    function openNewSalesMeetingForm() {
        ...
        if(meetingDateInput) {
            ...
            meetingDateInput.value = tomorrow.toISOString().slice(0,16); // Format for datetime-local
        }
        showSalesView('form');
    }
    ...
    function handleSaveMeeting(e) {
        ...
        setTimeout(() => {
            let meetingToSave;
            if (currentMeetingId) { // Editing existing meeting
                ...
                if (index > -1) {
                    ...
                    showNotificationCallback("Meeting updated successfully!", "success");
                     ...
                     showNotificationCallback("Error finding meeting to update.", "error");
                     if(saveMeetingBtn) setButtonLoadingStateCallback(saveMeetingBtn, false);
                    return;
                }
            } else { // Creating new meeting
                const newMeetingId = `sm-${generateIdCallback(8)}`; // Sales Meeting ID
                const newRecorderId = `rec-${generateIdCallback(8)}`; // Associated Recorder ID
                meetingToSave = {
                    id: newMeetingId,
                    title, date, clientEmail, notes,
                    status: 'Scheduled',
                    clientCode: generateIdCallback(6), 
                    recorderId: newRecorderId, 
                    recorderLink: generateRecorderLinkCallback(newRecorderId, generateIdCallback(8)), 
                    analysisAvailable: false,
                    analysisData: null // Ensure analysisData is null for new meetings
                };
                meetings.push(meetingToSave); // Add to local copy first
                showNotificationCallback("Meeting scheduled successfully!", "success");
            }
            
            saveMeetingsCallback(); // Persist all meetings via SharedAppLogic
    ...
    function viewSalesMeetingDetails(id) {
        const meeting = meetings.find(m => m.id === id);
        if (!meeting) {
            showNotificationCallback("Meeting details not found.", "error");
            return;
        }
        currentMeetingId = id; // Set for potential edit/delete actions
        ...
        if(detailsRecorderLinkAnchor) {
            ...
            detailsRecorderLinkAnchor.textContent = "Open Recorder Page"; // Or just the link
            // Store the actual link in a data attribute for the copy button
        ...
        // Fetch the most up-to-date analysis data from the shared source via recorderId
        ...
        if (recorderMeeting && recorderMeeting.status === 'completed' && recorderMeeting.analysisData) {
            ...
            populateSalesAnalysisData(recorderMeeting.analysisData); // Pass the actual analysis data
        } else {
        ...
        questionHistoryArray = []; // Reset for each meeting
        renderSalesQuestionHistory();
        if(questionResultWrapper) questionResultWrapper.classList.add('hidden');
        showSalesView('details');
    }
    
    function populateSalesAnalysisData(analysisData) { // Expects the actual analysisData object
    ...
    function handleDeleteSalesMeeting() {
        ...
        if (!meetingToDelete) {
             showNotificationCallback("Could not find meeting to delete.", "error");
            return;
        }
        if (!confirm(`Are you sure you want to delete the meeting "${meetingToDelete.title}"? This action cannot be undone.`)) return;
        
        meetings = meetings.filter(m => m.id !== currentMeetingId);
        saveMeetingsCallback(); 
        renderSalesMeetingList();
        showSalesView('list');
        currentMeetingId = null; // Reset
    ...
    function handleSalesQuestion(e){
        ...
        if(!q) {
            showNotificationCallback("Please type your query.", "warning");
            return;
        }
        ...
        setTimeout(() => {
            ...
            // Mock AI logic based on currentMeetingId and its analysisData
            const currentMeeting = meetings.find(m => m.id === currentMeetingId);
            const recorderMeeting = currentMeeting ? getMeetingByIdCallback(currentMeeting.recorderId) : null;

            if (recorderMeeting && recorderMeeting.analysisData) {
                if (q.toLowerCase().includes("concerns") && recorderMeeting.analysisData.summary) {
                    ans = `Based on the summary: ${recorderMeeting.analysisData.summary.split('. ')[0]}. For more, check Key Points.`;
                } else if (q.toLowerCase().includes("action items") && recorderMeeting.analysisData.actionItems) {
                    ans = `Identified Action Items: ${recorderMeeting.analysisData.actionItems.replace(/<[^>]+>/g, ' ').substring(0,100)}...`;
                } else if (q.toLowerCase().includes("sentiment") && recorderMeeting.analysisData.sentiment) {
                    ans = `Sentiment Analysis: ${recorderMeeting.analysisData.sentiment.replace(/<[^>]+>/g, ' ')}`;
                } else {
                    ans = "This is a sales-focused AI insight. The analysis data suggests focusing on follow-up actions.";
                }
            } else {
                ans = "Analysis data not available for this meeting to answer your query effectively.";
            }

            if(answerTextEl) answerTextEl.innerHTML = ans; // Use innerHTML if ans might contain HTML
    ...
    function setupEventListeners() {
        if (!newMeetingBtn) { // A check to ensure DOM elements are loaded
            console.error("Salesperson DOM not fully initialized for event listeners.");
            return;
        }
        ...
        if(logoutBtnSales) {
            logoutBtnSales.addEventListener('click', () => {
                // For "Logout Role", we just go back to the app dashboard.
                // Full logout is handled by app-main-dashboard or landing page.
        ...
        copyButtons.forEach(button => {
            button.addEventListener('click', (event) => {
                const strongEl = event.currentTarget.previousElementSibling.previousElementSibling; // Get the <strong> label
    ...
    return {
        init: (meetingsArr, notifyCb, switchCb, genIdCb, genRecLinkCb, saveCb, setLoadStateCb, getMeetingCb) => {
            ...
            // DOM references should be initialized after HTML is injected by the main page
            initDOMReferences(); 
            // Then setup event listeners
            setupEventListeners();
            
            renderSalesMeetingList();
            showSalesView('list'); // Initial view for Salesperson
            // console.log("Salesperson View Initialized with meetings:", meetings);
        },
        getHTML
    };
})();